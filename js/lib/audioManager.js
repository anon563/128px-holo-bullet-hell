class AudioManager {
    ...
    update = () => {
        if (!(Math.floor(this.frameCount) % 5)) this.seFrame = {};
        if (this.bgmFadeOut) {
    ...
    playSE = (id, variation) => {
        const sound = this.seList[id];
        if (this.seFrame[id] || this.seMuted || !sound || (sound && !sound.buffer)) return;
        this.seFrame[id] = true;
        ...
        sound.gainNode.gain.value = this.seMuted ? 0 : this.seVolume;
